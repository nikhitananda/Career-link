...
import { readAlert } from '../../services/form';
...
const Alerts = () => {
  ...
  useEffect(() => {
    const rawData = Object.entries(readAlert());
    console.log(rawData);
    const dummy = rawData.map((item) => {
      if (item[1].message)
        return {
          colors: colors[Math.floor(Math.random() * 3)],
        ...
        <CardBody className="">
          <div className="mt-3">
            {alerts.map((item, index) => (
              <Alert color={item.colors ? item.colors : 'primary'} key={index}>
